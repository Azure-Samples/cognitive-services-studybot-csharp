...
This sample is a LUIS-based web app bot (v3) that integrates a QnA Maker chat client. It can be either a stand-alone web app bot that you run/test in a desktop chat client emulator or in the Azure portal, or you can build this bot to be your main bot for the Study Bot sample. If you want to integrate this sample into a website (optional), it can be embedded if you put the endpoint URL found in the Azure portal in a `WebView`, but that is not pursued in this sample.
...
1. Create a [basic LUIS web app bot](https://docs.microsoft.com/en-us/azure/bot-service/bot-service-quickstart?view=azure-bot-service-3.0) in the [Azure portal](https://ms.portal.azure.com), but using the `Basic Bot (C#)` template in the "Create" panel. Once you select `Basic Bot (C#)`, select the `SDKv3` choice and then choose the `Language Understanding, C#` template. Be sure to choose `Select` at the bottom of that panel, then proceed to fill out the rest of the panel and finally select `Create`. Your app will take a few minutes to deploy. After this is created, Azure will automatically create a LUIS app (with the same name as your LUIS web app bot in Azure) in [luis.ai](https://www.luis.ai), where you will create intents later. From this Azure portal app, your LUIS keys (including your web app bot's Microsoft App ID and password) are generated. You will need these for the `Web.config` file in your bot.
